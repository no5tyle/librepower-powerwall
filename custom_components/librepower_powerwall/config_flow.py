...
Gateway-password only, for now
-------------------------------
This flow briefly also offered a "pair with my Tesla account, no password
needed" path (a PairingStepsMixin + a v1r-only setup step + an options flow
for pairing after the fact), built on pairing.py's Owner API login. That
login path is dead: Tesla decommissioned owner-api.teslamotors.com for
third-party callers in June 2026 (every call now returns 403 "forbidden, see
https://developer.tesla.com/docs/fleet-api"), which broke the only login
mechanism that flow depended on - registering a key, polling its state, and
the pure-v1r read/write transport (powerwall_v1r.py) were never the broken
part, but none of that is reachable without a working way to get a Tesla
access token first.

Reviving it needs the Fleet API instead - a registered developer app
(business account, ~5 business day approval, a real redirect URI), not just
a Tesla login. That's real added friction for anyone who wants control
writes, so it's being held back rather than shipped broken. pairing.py and
powerwall_v1r.py are left in place (not deleted) since the RSA
register/poll/keygen logic and the v1r client are still correct - they just
need a different way to obtain the access_token pairing.py currently gets
via the dead Owner API path (pairing.build_authorize_url/async_exchange_code
specifically). See those modules' own docstrings.
"""
